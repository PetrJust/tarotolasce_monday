...
function directionOpen(
  ...
  if (spread === "yesno") {
    // U jednokaretního výkladu kartu NEjmenujeme (celý výklad = ta karta).
    if (singleCard) {
      const pairs = [
        { open: `A právě tahle karta napovídá, kterým směrem se to celé kloní —`, close: `a taky proč to tak je.` },
        { open: `A právě tahle karta ukazuje, co tvou odpověď doopravdy rozhoduje —`, close: `víc než cokoli, co ti teď běží hlavou.` },
      ];
...
export function mockFlowB(
  ...
  // JEDNOKARETNÍ výklad (ano/ne): ta jediná karta JE celý výklad, takže
  // v úvodu zdarma NEjmenujeme kartu ani neprozradíme její význam - jinak
  // by uživatel dostal celý výklad zdarma. Místo toho jen naznačíme, že
  // je karta vyložená a odpověď se rýsuje. U vícekaretních výkladů zůstává
  // původní úvod (jmenuje první z více karet + jednu větu jako důkaz).
  const singleCard = cards.length === 1;
  const l2 = singleCard
    ? "Kartu, kterou sis vytáhla, už mám před sebou a odpověď se z ní začíná rýsovat."
    : firstCardSentence(cards[0], seed);
  const dir = directionOpen(spread, cards[0], seed, singleCard);
