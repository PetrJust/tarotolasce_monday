...
describe("v1.6 §9.2/9.3: Flow B úvod a kontinuita", () => {
  ...
  it("úvod končí uprostřed myšlenky (pomlčka); vícekaretní jmenuje první kartu, jednokaretní ne", () => {
    for (const [spread, q, cards] of cases) {
      ...
      if (cards.length > 1) {
        // vícekaretní: úvod jmenuje první z karet (ukázka, důkaz výkladu)
        expect(teaser).toContain("První karta, kterou sis vytáhla");
      } else {
        // jednokaretní (ano/ne): ta jediná karta JE celý výklad, úvod ji
        // proto NEjmenuje ani neprozradí její význam
        expect(teaser).not.toContain("První karta, kterou sis vytáhla");
      }
    }
  });
  ...
  it("jednokaretní úvod neprozradí kartu, význam ani směr (v1.6.1 §4)", () => {
    ...
    // a kontinuita drží
    expect(single.full.startsWith(single.teaser)).toBe(true);
  });
